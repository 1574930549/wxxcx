...
Page({
    ...
    searchBtne: function(){
        var that = this;
        var up='allcatslist['+this.data.n+']';
        if (that.data.searchkeyword.length > 0) {
            that.setData({
                [up]:that.data.searchkeyword,
                n:this.data.n+1
            })
            wx.navigateTo({
    ...
    searchinpute(e) {
        this.setData({
            searchkeyword: e.currentTarget.dataset.value
        })
        this.searchBtn()
    },
    /**
        ...
        Net.request({
            ...
            success: function(res) {
                ...
                for (let i = 0; i < datas.length; i++) {
                  datas[i].image = "../../resources/logo.jpg"
